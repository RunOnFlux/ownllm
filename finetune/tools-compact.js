...
module.exports = [
  ...
  { type: 'function', function: { name: 'flux_get_template', description: 'Exact specification of a marketplace template: image, ports, env, resources, instances and containerData. Use it instead of recalling these values.', parameters: { type: 'object', properties: { name: { type: 'string', description: 'exact template name' }, search: { type: 'string', description: 'free text when the name is unknown' }, category: { type: 'string', description: 'list a category' } } } } },
  { type: 'function', function: { name: 'flux_validate_spec', description: 'Ask a FluxOS node to verify a spec as it would at registration: image reachable and linux/amd64, ports usable, name free. Run before deploying.', parameters: { type: 'object', required: ['spec'], properties: { spec: SPEC } } } },
  { type: 'function', function: { name: 'flux_check_image', description: 'Check a Docker image and tag exist with a linux/amd64 build, and report size. Use whenever the image was not given verbatim or taken from a template.', parameters: { type: 'object', required: ['repotag'], properties: { repotag: { type: 'string' } } } } },
  { type: 'function', function: { name: 'flux_get_app_stats', description: 'Live CPU, memory and network use of an app. Use before advising on size instead of guessing.', parameters: { type: 'object', required: ['name'], properties: { name: { type: 'string' }, component: { type: 'string' } } } } },
